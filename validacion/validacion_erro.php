...
$old_error_handler = set_error_handler("validacion_scriptcase_error_handler");

function validacion_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['validacion']['contr_erro']) && $_SESSION['scriptcase']['validacion']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_validacion->contr_validacion->Apl_com_erro = true;
            if ($inicial_validacion->contr_validacion->NM_ajax_flag)
            {
                $inicial_validacion->contr_validacion->NM_ajax_info['errList']['geral_validacion'][] = $msg;
                return;
            }
            validacion_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function validacion_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['validacion'])  && '' != $_SESSION['scriptcase']['error_icon']['validacion'])  ? $_SESSION['scriptcase']['error_icon']['validacion']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['validacion']) && '' != $_SESSION['scriptcase']['error_close']['validacion']) ? $_SESSION['scriptcase']['error_close']['validacion'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['validacion']) && '' != $_SESSION['scriptcase']['error_span_title']['validacion']) ? $_SESSION['scriptcase']['error_span_title']['validacion'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['validacion']) && '' != $_SESSION['scriptcase']['error_icon_title']['validacion']) ? $_SESSION['scriptcase']['error_icon_title']['validacion'] : '';
?>
