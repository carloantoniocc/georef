<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="arc_codigo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['arc_codigo']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_archivos_1']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbl_archivos_1"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_ver_mapa()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_ver_mapa(); }, 500);
      return;
    }
    do_ajax_form_tbl_archivos_1_event_scajaxbutton_ver_mapa_onclick();
}
function sc_btn_ver_imagen()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_ver_imagen(); }, 500);
      return;
    }
    do_ajax_form_tbl_archivos_1_event_scajaxbutton_ver_imagen_onclick();
}
function sc_btn_descarga()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_descarga(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_tbl_archivos_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "codigo_archivo?#?" . str_replace("'", "@aspass@", $_SESSION['codigo_archivo']) . "?@?SC_glo_par_codigo_archivo?#?codigo_archivo?@?"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_tbl_archivos');
}
function sc_btn_contacto()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_contacto(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_form_tbl_contacto_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NM_btn_insert?#?S?@?NM_btn_update?#?S?@?NM_btn_delete?#?S?@?NM_btn_navega?#?N?@?"; ?>', '', '_blank', '0', '0', 'form_tbl_contacto');
}
function sc_btn_Mantenedor()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Mantenedor(); }, 500);
      return;
    }
  nm_gp_submit('<?php echo $this->Ini->link_archivos_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NM_btn_insert?#?S?@?NM_btn_update?#?S?@?NM_btn_delete?#?S?@?NM_btn_navega?#?N?@?"; ?>', '', '_blank', '0', '0', 'archivos');
}
function sc_btn_info()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_info(); }, 500);
      return;
    }
  tb_show('', '<?php echo $this->Ini->link_mensaje_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&script_case_session=<?php echo session_id() ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=450&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbl_archivos_1']['where_detal']))
  {
?>  
