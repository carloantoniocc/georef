<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="arc_codigo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['arc_codigo_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="archivos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_sc_btn_0()
{
  ...
  tb_show('', '<?php echo $this->Ini->link_form_tbl_clasificacion_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&script_case_session=<?php echo session_id() ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=650&width=650&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['archivos']['where_detal']))
  {
?>  
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_archivos_submit_form(y, z); 
} 
<?php
}
?>
