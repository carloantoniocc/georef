...
$old_error_handler = set_error_handler("archivos_scriptcase_error_handler");

function archivos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['archivos']['contr_erro']) && $_SESSION['scriptcase']['archivos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_archivos->contr_archivos->Apl_com_erro = true;
            if ($inicial_archivos->contr_archivos->NM_ajax_flag)
            {
                $inicial_archivos->contr_archivos->NM_ajax_info['errList']['geral_archivos'][] = $msg;
                return;
            }
            archivos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function archivos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['archivos'])  && '' != $_SESSION['scriptcase']['error_icon']['archivos'])  ? $_SESSION['scriptcase']['error_icon']['archivos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['archivos']) && '' != $_SESSION['scriptcase']['error_close']['archivos']) ? $_SESSION['scriptcase']['error_close']['archivos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['archivos']) && '' != $_SESSION['scriptcase']['error_span_title']['archivos']) ? $_SESSION['scriptcase']['error_span_title']['archivos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['archivos']) && '' != $_SESSION['scriptcase']['error_icon_title']['archivos']) ? $_SESSION['scriptcase']['error_icon_title']['archivos'] : '';
?>
