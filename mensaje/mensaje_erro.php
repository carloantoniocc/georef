...
$old_error_handler = set_error_handler("mensaje_scriptcase_error_handler");

function mensaje_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['mensaje']['contr_erro']) && $_SESSION['scriptcase']['mensaje']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_mensaje->contr_mensaje->Apl_com_erro = true;
            if ($inicial_mensaje->contr_mensaje->NM_ajax_flag)
            {
                $inicial_mensaje->contr_mensaje->NM_ajax_info['errList']['geral_mensaje'][] = $msg;
                return;
            }
            mensaje_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function mensaje_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['mensaje'])  && '' != $_SESSION['scriptcase']['error_icon']['mensaje'])  ? $_SESSION['scriptcase']['error_icon']['mensaje']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['mensaje']) && '' != $_SESSION['scriptcase']['error_close']['mensaje']) ? $_SESSION['scriptcase']['error_close']['mensaje'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['mensaje']) && '' != $_SESSION['scriptcase']['error_span_title']['mensaje']) ? $_SESSION['scriptcase']['error_span_title']['mensaje'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['mensaje']) && '' != $_SESSION['scriptcase']['error_icon_title']['mensaje']) ? $_SESSION['scriptcase']['error_icon_title']['mensaje'] : '';
?>
